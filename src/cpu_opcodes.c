...
const struct cpu_opcode opcode_table[256] = {
    ...
    [0x0A] = OP(ASL_A, MODE_IMP, 1, 2),  // ASL A with Accumulator as argument // MODE_ACC
    ...
    // Affects Flags: N Z C
    [0x4A] = OP(LSR_A, MODE_IMP, 1, 2),  // LSR A // MODE_ACC
    ...
    // ROL (ROtate Left)
    // Affects Flags: N Z C
    [0x2A] = OP(ROL_A, MODE_IMP, 1, 2),  // MODE_ACC
    ...
    // ROR (ROtate Right)
    // Affects Flags: N Z C
    [0x6A] = OP(ROR_A, MODE_IMP, 1, 2),  // MODE_ACC
